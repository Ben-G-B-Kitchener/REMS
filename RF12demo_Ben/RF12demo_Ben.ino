...
// EEPROM address range used by the avr_config() code
#define AVR_EEPROM_ADDR    ((uint8_t*) 0x60)  ///< Starting offset.
#define AVR_EEPROM_SIZE    16                 ///< Number of bytes.
#define AVR_EEPROM_EKEY    ((uint8_t*) 0x80)  ///< EE start, same as before.
#define AVR_EEPROM_ELEN    16                 ///< EE number of bytes.
#define AVR_EEPROM_VERSION 1                  ///< Only this version is valid.


//#define RF69_COMPAT 0 // 1=base node (RF69CW), 0=sensor nodes (RF12)
#define RF69_COMPAT 1 // 1=base node (RF69CW), 0=sensor nodes (RF12)
...
/*static void loadAVRConfig () {
    // eeprom_read_block(&config, RF12_EEPROM_ADDR, sizeof config);
    // this uses 166 bytes less flash than eeprom_read_block(), no idea why
    for (byte i = 0; i < sizeof configAVR; ++ i)
        ((byte*) &configAVR)[i] = eeprom_read_byte(AVR_EEPROM_ADDR + i);
}*/
...
static void saveAVRConfig () {
    // eeprom_write_block(&config, RF12_EEPROM_ADDR, sizeof config);
    // this uses 170 bytes less flash than eeprom_write_block(), no idea why
//    eeprom_write_byte(AVR_EEPROM_ADDR, ((byte*) &configAVR)[0]);
//    for (byte i = 0; i < sizeof configAVR; ++ i)
//        eeprom_write_byte(AVR_EEPROM_ADDR + i, ((byte*) &configAVR)[i]);
//    Serial.println(configAVR.tempCal);
    eeprom_write_word (0x60, configAVR.tempCal);
//    unsigned int test = eeprom_read_word(0x60);
//    Serial.println(test);
}
...
static void handleInput (char c) {
    ...
    if (c == '>') {
        ...
        case 'T': // set AVR internal temperature sensor calibration offset
            configAVR.tempCal = (unsigned int) value;
            saveAVRConfig();
            //Serial.println(configAVR.tempCal);
            break;  
...
void loop () {
    ...
    if (cmd && rf12_canSend()) {
        ...
        //Serial.print("stack = "); Serial.println(stack, BIN);
